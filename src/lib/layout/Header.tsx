...
const NavLink = ({ key, children }: { key: any; children: ILinks }) => (
  <Fragment key={key}>
    {children.child ? (
      <Menu>
        <MenuButton as={Button} rightIcon={<ChevronDownIcon />} fontWeight="normal" bg="white">
          {children.name}
        </MenuButton>
        <MenuList>
    ...
    <Box px={4}>
      <Flex h={16} alignItems="center" justifyContent="space-between">
        ...
        <HStack spacing={8} alignItems="center" justify="center" flexGrow={1}>
          ...
          <HStack as="nav" spacing={4} justify="center" flexGrow={1} display={{ base: 'none', md: 'flex' }}>
            {Links.map((link, index) => (
              <NavLink key={index.toString()}>{link}</NavLink>
            ))}
          </HStack>
        </HStack>
