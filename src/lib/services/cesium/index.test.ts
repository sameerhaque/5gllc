...
describe('CesiumService test suite', () => {
  ...
  test('createCesiumViewer method should be called', () => {
    const spy = vi.spyOn(cesiumService, 'createCesiumViewer').mockImplementation(mockCesiumImplementation)
    cesiumService.createCesiumViewer()
    expect(spy).toHaveBeenCalledTimes(1)
  })

  test('loadTileSets method should be called', () => {
    const spy = vi.spyOn(cesiumService, 'loadTileSets').mockImplementation(mockCesiumImplementation)
  ...
  test('flyToMarket method should be called', () => {
    ...
    const spy = vi.spyOn(cesiumService, 'flyToMarket').mockImplementation(mockCesiumImplementation)
