...
import { ICameraOrientation, IConfig, IMarket, IScreenSpaceEventFeature } from 'lib/interfaces'
...
export default class CesiumService {
  ...
  constructor(private _viewer: Cesium.Viewer) {
    ...
    this._screenSpaceEventHandler = new Cesium.ScreenSpaceEventHandler(this._viewer.canvas)
  ...
  createCesiumViewer(): void {
    ...
    ;(<HTMLElement>document.querySelector('.cesium-navigationHelpButton-wrapper')).style.visibility = 'hidden'
    ;(<HTMLElement>document.querySelector('.cesium-sceneModePicker-wrapper')).style.visibility = 'hidden'
    ;(<HTMLElement>document.querySelector('.cesium-viewer-geocoderContainer')).style.visibility = 'hidden'
    ;(<HTMLElement>document.querySelector('.cesium-viewer-toolbar')).style.visibility = 'hidden'
    ...
    this._screenSpaceEventHandler.setInputAction(({ position }: { position: Cesium.Cartesian2 }): void => {
      ...
      this._selectedFeature.feature = <Cesium.Cesium3DTileFeature | undefined>this._viewer.scene.pick(position)
    ...
    this._screenSpaceEventHandler.setInputAction(({ endPosition }: { endPosition: Cesium.Cartesian2 }): void => {
      if (this._highlightedFeature?.feature && this._highlightedFeature.feature !== this._selectedFeature.feature) {
        this._highlightedFeature.feature.color = this._highlightedFeature.originalColor
        this._highlightedFeature.feature = undefined
      }
      this._highlightedFeature.feature = <Cesium.Cesium3DTileFeature | undefined>this._viewer.scene.pick(endPosition)
      this._highlightedFeature?.feature &&
        this._highlightedFeature.feature instanceof Cesium.Cesium3DTileFeature &&
        (this._highlightedFeature.feature.color = this._highlightedFeature.highlightColor)
